...
function MainPopup({ isMobile, isTablet, isTouch }) {
  ...
  useEffect(() => {
    ...
    if (forcePopup) {
      popups.forEach((popup) => {
        localStorage.removeItem(`wd-main-popup-hidden-date:${popup.id}`);
      });
    }
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label={fallbackPopup.title || "메인 팝업"}
      style={{
        position: "fixed",
        inset: 0,
        zIndex: 9999,
      ...
      >
        ...
        <div style={{ padding: isMobile ? "20px 20px 16px" : "24px 26px 18px" }}>
          ...
          {fallbackPopup.body && (
            <p
              style={{
                margin: fallbackPopup.title ? "12px 0 0" : 0,
                color: "#4B5563",
...
export default function Home1() {
  ...
  useEffect(() => {
    ...
    fetchHomeNoticeItems()
      ...
      .finally(() => {
        if (alive) setLoadingNotices(false);
      });
  ...
  const getStoryThumbUrl = (thumbnail) => {
    ...
    const isCld =
      /res\.cloudinary\.com\//.test(src) && /\/image\/upload\//.test(src);
  ...
  white-space: nowrap;
    ...
    `,
      ...
      <main style={{ background: "#fff", overflowX: "hidden" }}>
        ...
        <Section
          fullBleed
          innerMaxWidth={isWide ? 1560 : CONTAINER}
          style={{
            paddingTop: isMobile ? 36 : isTablet ? 60 : isWide ? 72 : 80,
            paddingBottom: isMobile ? 40 : isTablet ? 72 : isWide ? 88 : 96,
          ...
          >
            ...
            >
              <div
                style={{
                  position: "relative",
                  height: isMobile ? 220 : isTablet ? 300 : isWide ? 410 : 360,
                  borderRadius: PALETTE.radiusLg,
                  overflow: "hidden",
                    ...
                    {quickLinks.map((it, i) => (
                      <Link
                        ...
                        style={{
                          ...
                          width: "100%",
                          minHeight: isTablet ? 64 : isWide ? 70 : 64,
                        }}
                        onMouseEnter={(e) => {
