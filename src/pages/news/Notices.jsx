        ...
        <div className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-lg">
          <table className="w-full text-sm">
            <thead className="bg-gray-50/90 backdrop-blur supports-[backdrop-filter]:bg-gray-50/75 text-gray-600 sticky top-0 z-10 border-b border-gray-100">
              <tr>
                <th className="w-20 py-3.5 pl-4 pr-2 text-center font-medium text-gray-600">번호</th>
                <th className="py-3.5 pl-6 pr-4 text-left font-medium text-gray-600">제목</th>
              </tr>
            </thead>
            <tbody>
              {paginatedItems.map((it, idx) => {
                ...
                return (
                  <tr key={it.slug} className="border-t border-gray-100 odd:bg-white even:bg-gray-50/40 hover:bg-gray-100/60 transition-colors">
                    <td className="py-4 pl-4 pr-2 text-gray-400 text-center align-top">{number}</td>
                    <td className="py-4 pl-6 pr-4 align-top">
                      <Link
                        to={`/news/notices/${encodeURIComponent(it.slug)}`}
                        className="inline-flex items-center hover:underline decoration-2 decoration-sky-300 underline-offset-2 max-w-[980px] truncate"
                      >
